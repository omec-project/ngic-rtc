...
//	uint8_t ebi_index = 0;
	...
	if ((ret = upf_context_entry_lookup(pdn->upf_ipv4.s_addr,
					&upf_ctx)) < 0) {
		clLog(clSystemLog, eCLSeverityCritical, "%s:Error: upx context not found %d %d\n", __func__, __LINE__,ret);
	...
	if(context->change_report == TRUE && pfcp_config.cp_type == SGWC) {
			...
			pdn->apn_restriction);
	{
		...
		for(uint8_t i= 0; i< MAX_BEARERS; i++) {
			...
			if (bearer->s11u_mme_gtpu_teid) {
				clLog(clSystemLog, eCLSeverityDebug,"S11U Detect- set_create_session_response-"
		...
		if (spgw_cfg != SAEGWC) {
			if ((context->pgw_fqcsid)->num_csid) {
				set_gtpc_fqcsid_t(&cs_resp.pgw_fqcsid, IE_INSTANCE_ZERO,
						context->pgw_fqcsid);
				//cs_resp.pgw_fqcsid.node_address = (context->pgw_fqcsid)->node_addr;
			}
		}
	}
