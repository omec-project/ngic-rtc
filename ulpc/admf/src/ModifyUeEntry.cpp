...
ModifyUeEntryPost :: ModifyUeEntryPost(ELogger &audit, AdmfApplication &app) 
: EManagementHandler( EManagementHandler::HttpMethod::httpPost,
			UPDATE_UE_ENTRY, audit ), mApp(app)
{
}

void
ModifyUeEntryPost :: process(const Pistache::Http::Request& request,
				Pistache::Http::ResponseWriter &response)
{
	...
	if (jsonReq.HasParseError()) {
		...
		ELogger::log(LOG_ADMF).info("Update Ue Request: Json parsing error. "
								"Invalid Json.");
	...
	itr = jsonReq.FindMember(REQUEST_SOURCE_KEY);
        if (itr != jsonReq.MemberEnd()) {
                ...
                ELogger::log(LOG_ADMF).debug("Update Ue request came from D_ADMF.");
	...
	mApp.getAdmfController().modifyUeController(ueEntries,
				requestBody, requestSource);
	ELogger::log(LOG_ADMF).debug("Response: Update Ue Entry processed successfully.");
	response.send(Pistache::Http::Code::Ok, "Update Ue Entry processed sucessfully\n");
